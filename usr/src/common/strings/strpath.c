...
strpath_append(custr_t *path, const char *inp)
{
	...
	if (pathl > 0) {
		/*
		 * If the existing path does not end in a slash, we must append
		 * one now.
		 */
		...
		/*
		 * If there is no existing path, and the input path is fully
		 * qualified, ensure that the output path is also.
		 */
		if (inp[0] == '/') {
			if (custr_appendc(path, '/') != 0) {
				return (-1);
	...
	/*
	 * Append characters from the input path, but do not copy two slashes
	 * in a row or write a trailing slash:
	 */
	for (;;) {
		char c = inp[i++];
