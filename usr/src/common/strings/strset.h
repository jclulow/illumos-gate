...
/*
 * String Set (strset_t) data structure backed by libavl.
 */
...
typedef enum strset_walk {
	/*
	 * What action should the walker take next?
	 */
	...
	/*
	 * If REMOVE is logically OR-ed with one of the next actions (above),
	 * the walker will remove this entry from the set before continuing.
	 * Useful for trimming filtered entries from the set.
	 */
...
typedef strset_walk_t strset_walk_func(strset_t *, const char *, void *,
    void *);
