...
struct xxx_print_stuff {
	unsigned xps_count;
	unsigned xps_disk_count;

	unsigned xps_all;

	struct xxx_majors xps_majors[32]; /* XXX */
	unsigned xps_nmajors;
};

static void
xxx_config_all(struct xxx_print_stuff *xps)
{
	printf("START xxx_config_all: (%u majors)\n", xps->xps_nmajors);

	for (unsigned i = 0; i < xps->xps_nmajors; i++) {
		struct xxx_majors *xm = &xps->xps_majors[i];

		printf("   --- type %s major %d\n", xm->xm_type, xm->xm_major);
		int r = ndi_devi_config_driver(ddi_root_node(),
		    NDI_CONFIG | NDI_NO_EVENT, xm->xm_major);
		printf("     -> %d\n", r);
	}

	printf("DONE xxx_config_all\n");
}

static void
xxx_record_major(struct xxx_print_stuff *xps, major_t major, const char *t)
{
	printf("xxx_record_major(%d, %s)\n", major, t);

	/*
	 * First, check to see if this major appears already.
	 */
	for (unsigned i = 0; i < xps->xps_nmajors; i++) {
		if (xps->xps_majors[i].xm_major == major) {
			printf("   already in slot %u\n", i);
			return;
		}
	}

	printf("   adding to slot %u\n", xps->xps_nmajors);
	struct xxx_majors *xm = &xps->xps_majors[xps->xps_nmajors++];
	xm->xm_used = 1;
	xm->xm_major = major;
	xm->xm_type = t;
}
...
static int
xxx_print_stuff(dev_info_t *dip, void *arg)
{
	struct xxx_print_stuff *xps = arg;
	char buf[1024];
	...
	if (xps->xps_all) {
		char **class_strings;
		int n = i_ddi_get_exported_classes(dip, &class_strings);
		for (int i = 0; i < n; i++) {

			const char *cs = class_strings[i];

			if (strncmp(cs, "scsi", 4) == 0) {
				printf("XXX WALK EC SCSI: %s\n", ddi_pathname(dip, buf));
				xxx_record_major(xps, ddi_driver_major(dip), "SCSI");
			} else if (strcmp(cs, "fibre-channel") == 0) {
				printf("XXX WALK EC FC: %s\n", ddi_pathname(dip, buf));
				xxx_record_major(xps, ddi_driver_major(dip), "FC");
			} else if (strcmp(cs, "disk") == 0) {
				printf("XXX WALK EC DISK: %s\n", ddi_pathname(dip, buf));
				xxx_record_major(xps, ddi_driver_major(dip), "DISK");
			} else {
				continue;
			}

#if 0
			printf("   attempting i_ndi_config_node()\n");
			int r = i_ndi_config_node(dip, DS_READY, 0);
			printf("         -> %d\n", r);
...
#if 0
			/*
			 * Attempt to configure and attach this node.
			 */
			int r = ndi_devi_config(dip,
			    NDI_CONFIG | NDI_DEVI_PERSIST | NDI_NO_EVENT | NDI_DRV_CONF_REPROBE);
			printf("        ndi_devi_config -> %d\n", r);
			if (!i_ddi_devi_attached(dip)) {
				printf("        (not attached)\n");
			}
			...
			if (pdip != NULL) {
				printf("           trying parent %s\n", ddi_pathname(pdip, buf));
				r = ndi_devi_config(pdip,
				    NDI_CONFIG | NDI_DEVI_PERSIST | NDI_NO_EVENT | NDI_DRV_CONF_REPROBE);
				printf("        ndi_devi_config -> %d\n", r);
		...
		i_ddi_free_exported_classes(class_strings, n);
	}

	if (i_ddi_devi_class(dip) == NULL ||
	    strcmp(i_ddi_devi_class(dip), /* XXX ESC_DISK */ "disk") != 0) {
		/*
		 * We don't think this is a disk.
		 */
		return (DDI_WALK_CONTINUE);
	}

	printf("XXX WALK DISK: %s\n", ddi_pathname(dip, buf));
	xps->xps_disk_count++;
...
main(void)
{
	...
	debug_enter("after mountroot"); /* XXX */
