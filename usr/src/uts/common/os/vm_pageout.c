...
static pgcnt_t
tune(pgcnt_t initval, pgcnt_t ceiling, pgcnt_t defval)
{
	if (initval == 0 || initval >= ceiling) {
		return (defval);
	} else {
		return (initval);
	}
...
setupclock(void)
{
	 ...
	 *
	 * By default, we make lotsfree 1/64th of the total memory in the
	 * machine.  We also mandate a minimum of 32MB, to ensure that we leave
	 * enough in throttlefree and pageout_reserve (9MB) for the overhead
	 * incurred by ZFS during pageout activity.  On a 512MB system, this
	 * would represent just over 6% of system memory.
	 */
	lotsfree = tune(clockinit.ci_lotsfree, looppages,
	    MAX(looppages / 64, btop(32 * MEGABYTES)));

	desfree = tune(clockinit.ci_desfree, lotsfree, lotsfree / 2);

	minfree = tune(clockinit.ci_minfree, desfree, 3 * desfree / 4);

	throttlefree = tune(clockinit.ci_throttlefree, desfree, minfree);
...
uint_t dopageout = 1;	/* must be non-zero to turn page stealing on */
